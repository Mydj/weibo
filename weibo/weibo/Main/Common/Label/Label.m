...
#define LaBEL_IMAGE_NAME @"imageName"
@interface Label ()
...
- (void)drawRect:(CGRect)rect
{
    ...
    //自定义当前超链接文本颜色
    if ([self.LabelDelegate respondsToSelector:@selector(linkColorWithLabel:)]) {
        self.linkColor = [self.LabelDelegate linkColorWithLabel:self];
    }
    
    //当前文本超链接文字手指经过的颜色默认为greenColor
    self.passColor = [UIColor greenColor];
    //自定义当前超链接文本颜色
    if ([self.LabelDelegate respondsToSelector:@selector(passColorWithLabel:)]) {
        self.passColor = [self.LabelDelegate passColorWithLabel:self];
    }
    if (self.text == nil) {
        return;
    }
    ...
    //----------------------设置链接文本的颜色-------------------
    //判断当前链接文本表达式是否实现
    if ([self.LabelDelegate respondsToSelector:@selector(contentsOfRegexStringWithLabel:)] && [self.LabelDelegate contentsOfRegexStringWithLabel:self] != nil)
    {
        //获取所有的链接文本
    ...
    for (int i = 0; i < CFArrayGetCount(lines); i++) {
        ...
        for (int j = 0; j < CFArrayGetCount(runs); j++) {
            ...
            NSString *imageName = [attributes objectForKey:LaBEL_IMAGE_NAME];
            //图片渲染逻辑
            if (imageName) {
...
- (NSArray *)imagesOfRegexStrArray
{
    ...
    if ([self.LabelDelegate respondsToSelector:@selector(imagesOfRegexStringWithLabel:)]) {
        regex = [self.LabelDelegate imagesOfRegexStringWithLabel:self];
    }
...
- (void)replaceImageText
{
    ...
    for (NSString *imageUrl in [self imagesOfRegexStrArray]) {
        ...
        //设置空格的属性
        [imageAttributedString addAttribute:LaBEL_IMAGE_NAME value:imgName range:NSMakeRange(1, 1)];
...
- (NSArray *)contentsOfRegexStrArray
{
    ...
    NSString *regex = [self.LabelDelegate contentsOfRegexStringWithLabel:self];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (range.length == 0) {
        ...
        //判断当前代理方法是否实现
        if ([self.LabelDelegate respondsToSelector:@selector(toucheEndLabel:withContext:)]) {
            //获取当前点击字符串
            NSString *context = [[self.attrString string] substringWithRange:range];
            //调用点击开始代理方法
            [self.LabelDelegate toucheEndLabel:self withContext:context];
        }
    }
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    {
        //判断当前代理方法是否实现
        if ([self.LabelDelegate respondsToSelector:@selector(toucheBenginLabel:withContext:)]) {
            //获取当前点击字符串
            NSString *context = [[self.attrString string] substringWithRange:range];
            //调用点击开始代理方法
            [self.LabelDelegate toucheBenginLabel:self withContext:context];
        }
    }
...
+ (float)getTextHeight:(float)fontSize
                 width:(float)width
                  text:(NSString *)text
{
    Label *label = [[Label alloc] initWithFrame:CGRectMake(0, 0, width, 400)];
    label.font = [UIFont systemFontOfSize:fontSize];
    label.text = text;
    [label drawRect:CGRectMake(0, 0, width , 400)];
